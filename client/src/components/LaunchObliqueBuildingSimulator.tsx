...
interface LaunchObliqueBuildingSimulatorProps {
  v0: number;
  angle: number; // em graus
  h0: number; // altura do prédio em metros
  isRunning: boolean;
  resetTrigger: number;
}

export function LaunchObliqueBuildingSimulator({
  v0,
  angle,
  h0,
  isRunning,
  resetTrigger,
}: LaunchObliqueBuildingSimulatorProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Posição do objeto
      const x = buildingX + vx * t * scale;
      let y = buildingY - 60 - (vy0 * t * scale - 0.5 * g * t * t * scale);
      ...
      if (x < width - 50) {
        ...
        if (frameCount > 0 && t < tTotal) {
          ...
          for (let i = 0; i <= frameCount; i++) {
            ...
            const tx = buildingX + vx * ti * scale;
            const ty = buildingY - 60 - (vy0 * ti * scale - 0.5 * g * ti * ti * scale);
      ...
      ctx.fillText(`Lançamento Oblíquo de Prédio`, 10, 25);
      ctx.fillText(`v₀ = ${v0.toFixed(1)} m/s, θ = ${angle.toFixed(0)}°, h₀ = ${h0.toFixed(1)} m`, 10, 40);
      ctx.fillText(`h_máx = ${hMax.toFixed(2)} m`, 10, 55);
      ctx.fillText(`t_subida = ${tSubida.toFixed(2)} s`, 10, 70);
      ctx.fillText(`t_total = ${tTotal.toFixed(2)} s`, 10, 85);
      ctx.fillText(`Alcance = ${alcance.toFixed(2)} m`, 10, 100);

      // Tempo atual
      ctx.fillStyle = "#0284c7";
      ctx.font = "bold 12px Arial";
      ctx.fillText(`t = ${t.toFixed(2)} s`, 10, 117);
